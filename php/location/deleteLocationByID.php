<?php
	...
	try {
		...
		$departmentQuery = $conn->prepare('SELECT id FROM `department` WHERE locationID = ?');
		...
		if($departmentResult->num_rows > 0) {
			...
			$output['status']['name'] = "executed";
			$output['status']['description'] = "Unable to delete Location as it is used with Department";
			$output['data'] = [];
			mysqli_close($conn);
			echo json_encode($output); 
			exit;
